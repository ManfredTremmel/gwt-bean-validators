...
import de.knightsoftnet.validators.shared.impl.MustBeSmallerValidator;
 ...
 * </ul>
 * the entry of <code>field1</code> must be smaller then entry of <code>field2</code>, can be used
 * e.g. for date or number "from"/"to" fields.<br>
 ...
 */
@Documented
@Constraint(validatedBy = MustBeSmallerValidator.class)
@Target({ElementType.TYPE, ElementType.ANNOTATION_TYPE})
@Retention(RetentionPolicy.RUNTIME)
public @interface MustBeSmaller {
  /**
   * localized message.
   *
   * @return localized validation message
   */
  String message() default "{deKnightsoftnetValidatorsSharedValidationMustBeSmallerMessage}";
  ...
  /**
   * Defines several {@code MustBeSmaller} annotations on the same element.
   */
  @Target({ElementType.TYPE, ElementType.ANNOTATION_TYPE})
  @Retention(RetentionPolicy.RUNTIME)
  @Documented
