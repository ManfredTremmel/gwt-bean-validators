...
public class DateTimeLocalRenderer extends AbstractRenderer<Date> {
  ...
  public static final Renderer<Date> instance() { // NOPMD it's thread save!
    if (DateTimeLocalRenderer.instanceRenderer == null) {
      synchronized (DateTimeLocalRenderer.class) {
        if (DateTimeLocalRenderer.instanceRenderer == null) {
          DateTimeLocalRenderer.instanceRenderer =
              new DateTimeLocalRenderer("yyyy-MM-dd'T'HH:mm:ss");
        }
      }
    }
    return DateTimeLocalRenderer.instanceRenderer;
  }
