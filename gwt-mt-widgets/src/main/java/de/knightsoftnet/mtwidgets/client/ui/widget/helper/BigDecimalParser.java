...
public class BigDecimalParser implements Parser<BigDecimal> {
  ...
  private static volatile BigDecimalParser instanceParser = null;
  ...
  public static final Parser<BigDecimal> instance() { // NOPMD it's thread save!
    if (BigDecimalParser.instanceParser == null) {
      synchronized (BigDecimalParser.class) {
        if (BigDecimalParser.instanceParser == null) {
          BigDecimalParser.instanceParser = new BigDecimalParser();
        }
      }
    }
    return BigDecimalParser.instanceParser;
  }
  ...
  public final BigDecimal parse(final CharSequence pobject) throws ParseException {
    ...
    try {
      return BigDecimal.valueOf(NumberFormat.getDecimalFormat().parse(pobject.toString()));
