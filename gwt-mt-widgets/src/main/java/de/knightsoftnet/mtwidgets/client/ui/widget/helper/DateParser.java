...
public class DateParser implements Parser<Date> {
  ...
  private static volatile DateParser instanceParser = null;
  ...
  public static final Parser<Date> instance() { // NOPMD it's thread save!
    if (DateParser.instanceParser == null) {
      synchronized (DateParser.class) {
        if (DateParser.instanceParser == null) {
          DateParser.instanceParser = new DateParser("yyyy-MM-dd");
        }
      }
    }
    return DateParser.instanceParser;
  }

  public DateParser(final String pformat) {
    super();
